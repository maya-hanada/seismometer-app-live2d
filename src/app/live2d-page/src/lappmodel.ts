...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    motionUpdated !=
      this._quakeMotion.updateMotion(this._model, deltaTimeSeconds);
  ...
  /**
   * 引数で指定した地震モーションの再生を開始する
   * @param no グループ内の番号
   * @param priority 優先度
   * @param onFinishedMotionHandler モーション再生終了時に呼び出されるコールバック関数
   * @return 開始したモーションの識別番号を返す。個別のモーションが終了したか否かを判定するisFinished()の引数で使用する。開始できない時は[-1]
   */
  public startQuakeMotion(
    no: number,
    priority: number,
    onFinishedMotionHandler?: FinishedMotionCallback
  ): CubismMotionQueueEntryHandle {
    if (priority == LAppDefine.PriorityForce) {
      this._quakeMotion.setReservePriority(priority);
    } else if (!this._quakeMotion.reserveMotion(priority)) {
      if (this._debugMode) {
        LAppPal.printMessage("[APP]can't start motion.");
      }
      return InvalidMotionQueueEntryHandleValue;
    ...
    const motionFileName = this._modelSetting.getMotionFileName(
      LAppDefine.MotionQuake,
      no
    );

    // ex) idle_0
    const name = `${LAppDefine.MotionQuake}_${no}`;
    let motion: CubismMotion = this._motions.getValue(name) as CubismMotion;
    let autoDelete = false;
    ...
    //voice
    const voice = this._modelSetting.getMotionSoundFileName(
      LAppDefine.MotionQuake,
      no
    );
    if (voice.localeCompare('') != 0) {
    ...
    if (this._debugMode) {
      LAppPal.printMessage(`[APP]start quake motion: [${no}`);
    }
    return this._quakeMotion.startMotionPriority(motion, autoDelete, priority);
  }
  ...
  public constructor() {
    ...
    this._quakeMotion = new CubismMotionManager();
  }
  ...
  _quakeMotion: CubismMotionManager;
}
